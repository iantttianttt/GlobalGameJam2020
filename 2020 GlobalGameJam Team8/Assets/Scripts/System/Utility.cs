...
public class Utility
{
    ...
    public struct AssetRelate
    {
        ...
        /// Resources.LoadAll
        /// </summary>
        public static T[] ResourcesLoadAllCheckNull<T>(string name) where T : UnityEngine.Object
        {
            T[] loadGo = Resources.LoadAll<T>(name);

            if (loadGo.Length == 0)
            {
                Debug.LogError("Resources.LoadAll [ " + name + " ] Can't find any Assets !!");
                return default(T[]);
            }
